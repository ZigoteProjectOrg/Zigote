...
///     fixed loop and drawn by the viewport alongside editor-authored
///     <see cref="NodeKind.VfxEmitter" />
///     nodes. Mirrors <c>RuntimeAudioBackend</c> / <c>RuntimeInstancingBackend</c>. Fire-and-forget
///     emitters
...
public sealed class RuntimeVfxBackend : IVfxBackend
{
    ...
    {
        if (_emitters.TryGetValue(key: handle.Id, value: out var sim)) sim.Position = position;
    }

    public void SetEmitting(VfxHandle handle, bool emitting)
    {
        if (_emitters.TryGetValue(key: handle.Id, value: out var sim)) sim.Emitting = emitting;
    }

    public void Burst(VfxHandle handle, int count)
    {
        if (_emitters.TryGetValue(key: handle.Id, value: out var sim)) sim.Burst(count);
    }

    public void Destroy(VfxHandle handle) => _emitters.Remove(handle.Id);
    ...
    public void Step(float dt)
    {
        ...
        List<uint>? dead = null;
        foreach ((uint id, var sim) in _emitters)
        {
            sim.Tick(dt);
        ...
        if (dead == null) return;
        foreach (uint id in dead) _emitters.Remove(id);
    }
}
