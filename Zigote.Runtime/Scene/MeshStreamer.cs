...
///     <see cref="FileBytesLoader" />) when it comes into range and drops it when it goes out. The blob
...
///     <para>
///         Dependencies are injected (register-path, upload, unload) so the mechanism is headless-testable
///         against a real <see cref="AssetManager" /> without the native engine. Idempotent: <see cref="Want" />
...
public sealed class MeshStreamer : LodSystem.IResidencySink
{
    ...
    public void Want(SceneNode node, float distance)
    {
        ...
        if (!_handles.TryGetValue(node.Id, out var handle))
        {
            var id = _resolve(node);
            if (id.IsEmpty) return;
            handle = _assets.Acquire(id, FileBytesLoader.Instance);
        ...
        if (handle.IsLoaded && _uploaded.Add(node.Id))
            _upload(node, handle.Value!);
    }

    public void Drop(SceneNode node)
    {
        if (_handles.Remove(node.Id, out var handle))
