...
/// <summary>
///     The in-app devtools overlay: a <see cref="DevToolsView" /> pushed as its own overlay while
///     open.
///     <para>
...
///         header's expand button, or any phone-width screen, where a 408px column would be most of
///         the
...
public sealed class DevToolsPanel : ComposedWidget, IDismissableOverlay, INoAutoFocus
{
    ...
    public DevToolsPanel(DevToolsController controller) => _controller = controller;
    ...
    public float VisibleWidth => _full
        ? MathF.Max(x: Bounds.Width, y: _controller.DockWidth)
    ...
    private Widget BuildArm(BuildContext context, WindowSizeClass cls)
    {
        ...
        bool full = _controller.Fullscreen || cls == WindowSizeClass.Compact;
        float width = full ? mq.Width : _controller.DockWidth;
        _full = full;

        var view = new DevToolsView(
            controller: _controller,
            chrome: full ? DevToolsChrome.Fullscreen : DevToolsChrome.Docked
        );
        ...
        if (!full)
        {
            stack.Children.Add(
                new Positioned(
                    child: new DevResizeHandle(_controller),
                    top: 0,
                    bottom: 0,
                    right: width,
...
///     hairline, tinting to the accent while hovered or dragged, and carries a horizontal-resize
///     cursor
...
public sealed class DevResizeHandle(DevToolsController controller) : LeafWidget
{
    ...
    public override void Paint(PaintList paint)
    {
        ...
        paint.AddRect(
            ...
            color: active ? _theme.Accent : p.HeaderbarShade
        );
    }
    ...
    public override MouseCursor? GetCursor(Offset point) => MouseCursor.ResizeEW;
