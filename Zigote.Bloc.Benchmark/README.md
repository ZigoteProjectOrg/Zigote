...
Every row exists because something in `Zigote.Bloc/README.md` promises it. A promise nobody measured is a promise
that quietly stops being true — `AddWithoutEmitting` must allocate zero, `AddSync` must be one state record, and
...
constant, so **Mean reads directly as nanoseconds per event** and rows are comparable straight down. Flat means
adding producers is free; rising means they are serialising.
...
- **No head-to-head against another C# bloc library.** `Zigote.Reactive.Benchmark` has one against SignalsDotnet
  because a rival reactive core exists to port the shapes from. There is no equivalent .NET bloc to port; the channel
  pump in `Baseline.cs` is the honest stand-in for the dispatch model that is actually being argued against.
...
- **No correctness-under-contention probe.** That lives in `Zigote.Tests/BlocConcurrencyTests.cs` as assertions,
  because unlike a third-party library's behaviour, these are guarantees this repo makes and has to keep.
