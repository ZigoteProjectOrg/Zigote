...
///     <para>
///         <b>Total work is held constant</b> (<see cref="TotalOps" /> events however many threads share
///         them) and <c>OperationsPerInvoke</c> is that same constant, so the Mean column reads directly
...
///         <c>_pumping</c> drains for <i>everyone</i>, so the others enqueue and leave while one thread
///         does all the handling. That is the intended design — it is what keeps handlers from running
///         concurrently — but it means throughput past one producer is bounded by a single consumer, and
///         this benchmark is where that ceiling becomes visible instead of surprising.
///     </para>
///     <para>Run: <c>dotnet run -c Release --project Zigote.Bloc.Benchmark -- --filter *Contention*</c></para>
/// </summary>
[MemoryDiagnoser]
...
public class BlocContentionBenchmarks
{
    ...
    public void Setup()
    {
        ...
        for (var i = 0; i < Threads; i++) _disjoint[i] = new DrainCounter();
    }
    ...
    public void DisjointBlocs()
    {
        ...
        Parallel.For(
            ...
            t =>
            {
                ...
                for (var i = 0; i < perThread; i++) bloc.Add(Bump.One);
            }
        );
    }
    ...
    public void SharedBloc()
    {
        ...
        Parallel.For(
            ...
            _ =>
            {
                for (var i = 0; i < perThread; i++) _shared.Add(Bump.One);
            }
        );
...
/// <summary>
///     Out-of-process, because a contended invocation is far too slow for the in-process toolchain, and
///     every <see cref="BlocContentionBenchmarks.Threads" /> value runs under one job so the rows stay
///     comparable.
///     <para>
///         Wider than <see cref="Job.ShortRun" />, which is what the reactive project's equivalent uses:
///         at three iterations the margins here came back as much as 115% of the mean, which is not a
///         measurement — thread scheduling adds variance that a signal write on one thread does not have.
...
{
    public BlocContentionConfig()
    {
        AddJob(Job.Default.WithIterationCount(15).WithWarmupCount(10).WithLaunchCount(1));
    }
}
