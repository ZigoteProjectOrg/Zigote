...
public readonly struct ChartValue : IEquatable<ChartValue>
{
    ...
    public static ChartValue Number(double value) => new(
        kind: ChartValueKind.Number,
        number: value,
        category: null
    );

    public static ChartValue Category(string value) => new(
        kind: ChartValueKind.Category,
        number: 0,
        category: value
    );
    ...
    public static ChartValue Time(DateTime value)
    {
        return new ChartValue(
            kind: ChartValueKind.Time,
            number: value.Ticks / (double)TimeSpan.TicksPerSecond,
            category: null
        );
    }

    public static implicit operator ChartValue(double v) => Number(v);

    public static implicit operator ChartValue(float v) => Number(v);

    public static implicit operator ChartValue(int v) => Number(v);

    public static implicit operator ChartValue(long v) => Number(v);

    public static implicit operator ChartValue(string v) => Category(v);

    public static implicit operator ChartValue(DateTime v) => Time(v);
    ...
    public DateTime DateTime => Kind == ChartValueKind.Time
        ...
        ? new DateTime(
            Math.Clamp(
                value: (long)(Numeric * TimeSpan.TicksPerSecond),
                min: 0L,
    ...
    public bool Equals(ChartValue other) => Kind == other.Kind && Numeric.Equals(other.Numeric) &&
                                            _category == other._category;

    public override bool Equals(object? obj) => obj is ChartValue v && Equals(v);
    ...
    public static bool operator ==(ChartValue a, ChartValue b) => a.Equals(b);
    ...
    public override string ToString()
    {
        return Kind switch {
            ...
            _ => Numeric.ToString(format: "G9", provider: CultureInfo.InvariantCulture),
        };
    }
}
