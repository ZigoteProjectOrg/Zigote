...
///         </item>
///         <item>
///             <b>Sub-rectangle damage (GPU scissor).</b> When the <em>only</em> thing that changed this
///             frame is a small, precisely-known region (a blinking caret, a value-drag), the accumulated
///             <see cref="Damage" /> rects are handed to native, which repaints just those rects into the
///             persistent scene texture (<c>loadOp = load</c> + scissor) instead of clearing + redrawing
...
///     </list>
///     <para>
///         <b>Damage is frame-global and conservative.</b> Any change whose region is not precisely known
///         — every path that goes through <see cref="MarkAll" />/<see cref="MarkRoot" />/
///         <see cref="MarkOverlay" /> (discrete input, layout, animation, continuous mode, debug/snackbar/
...
///     </para>
///     <para>
///         Rects are kept pairwise non-overlapping (overlapping additions are merged into their bounding
///         union) so native can replay ops once per rect without double-blending the shared pixels.
///     </para>
/// </summary>
public sealed class RepaintTracker
{
    /// <summary>Upper bound on tracked disjoint damage rects; overflow degrades to <see cref="FullDamage" />.</summary>
    ...
    /// <summary>Anything to paint this frame? When false (and not in a continuous mode) the frame is skipped.</summary>
    ...
    /// <summary>
    ///     True when the whole frame must be repainted (the safe default). When false, <see cref="Damage" />
    ...
    public ReadOnlySpan<Rect> Damage => FullDamage ? default : _damage.AsSpan(0, _damageCount);
    ...
    private void Accumulate(Rect region)
    {
        ...
        while (true)
        {
            ...
            for (var i = 0; i < _damageCount; i++)
            {
                ...
                region = Rect.Union(_damage[i], region);
