...
///         skip the child cascade — but that cascade is already a no-op over an empty list, and
///         skipping
///         the base also skipped the mount lifecycle (so an <see cref="Widget.OnMount" /> that binds a
///         ticker never ran, leaving glyph animations dead) and <c>Owner.NotifyDetached</c> (so
///         removing
