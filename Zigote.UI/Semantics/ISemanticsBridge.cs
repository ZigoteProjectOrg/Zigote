...
/// <summary>
///     The seam between Zigote's platform-neutral <see cref="SemanticsNode" /> tree and a host platform's
///     native accessibility API (macOS <c>NSAccessibility</c>, Windows UI Automation, Linux AT-SPI/ATK).
///     The app builds + diffs the semantics tree in managed code; a bridge implementation is responsible
///     for projecting it onto the OS so screen readers (VoiceOver / Narrator / Orca) can read the UI and
...
///         which case the tree is still built on demand for the in-engine Semantics inspector + tests but
