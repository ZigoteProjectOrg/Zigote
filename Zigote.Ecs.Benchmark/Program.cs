...
internal static class Program
{
    ...
    private static readonly (ulong ent, float x)[] _ring = new (ulong, float)[ObsPerFrame * 2];
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            PerEntityFfiBenchmark(w: w, ents: ents);
            w.Dispose();
    ...
    private static (EcsWorld, Entity[]) SpawnBenchmark()
    {
        ...
        double allocKb = (GC.GetAllocatedBytesForCurrentThread() - alloc0) / 1024.0;
        Report(label: $"create + 2x Set  ({N:n0} entities)", ops: N, ms: ms);
        Console.WriteLine(
    ...
    private static void IterationBenchmark(EcsWorld w)
    {
        ...
        using (var q = w.Query<Position, Velocity>())
        {
            ...
            double ms = TimeMs(() =>
                {
                    for (int f = 0; f < Frames; f++) q.Each(MoveChunk);
                }
            );
            long alloc = GC.GetAllocatedBytesForCurrentThread() - a0;
            Report(
                label: $"Query.Each chunk iter  ({N:n0} x {Frames} frames)",
                ops: updates,
                ms: ms
            );
        ...
        long before = SumX(w);
        w.RegisterSystem<Position, Velocity>(
            name: "Move",
        ...
        double ms2 = TimeMs(() =>
            {
                for (int f = 0; f < Frames; f++) w.Progress();
            }
        );
        long after = SumX(w);
        long expectedDelta =
            (long)N * (Frames + 1); // +1 warmup, each frame adds vel.X=1 per entity
        bool ran = after - before == expectedDelta;
        if (ran)
        {
            Report(
                label: $"Pipeline Progress()    ({N:n0} x {Frames} frames)",
                ops: updates,
                ms: ms2
            );
        }
    ...
    private static void PerEntityFfiBenchmark(EcsWorld w, Entity[] ents)
    {
        ...
        double msSet = TimeMs(() =>
            {
                for (int i = 0; i < N; i++) w.Set(e: ents[i], c: new Position { X = i });
            }
        );
        Report(label: $"Set<Position>  ({N:n0} ops)", ops: N, ms: msSet);
        ...
        double msGet = TimeMs(() =>
            {
                for (int i = 0; i < N; i++)
                {
                    w.TryGet<Position>(e: ents[i], value: out var p);
                    acc += p.X;
                }
            }
        );
        _sink += acc;
        Report(label: $"TryGet<Position>  ({N:n0} ops)", ops: N, ms: msGet);
        Console.WriteLine();
    }
    ...
    private static void ObserverBenchmark()
    {
        ...
        Collect();
        double msBase = TimeMs(() =>
            {
                for (int f = 0; f < ObsFrames; f++)
                for (int k = 0; k < ObsPerFrame; k++)
                    wb.Set(e: eb[k], c: new Position { X = f });
            }
        );
        wb.Dispose();
        Report(
            label: $"Set baseline, no observer  ({totalSets:n0} sets)",
            ops: totalSets,
            ms: msBase
        );
        ...
        Collect();
        long allocA = GC.GetAllocatedBytesForCurrentThread();
        long drainedTotal = 0;
        long drainTicks = 0;
        float dacc = 0;
        double msObs = TimeMs(() =>
            {
                for (int f = 0; f < ObsFrames; f++)
                {
                    _ringCount = 0;
                    for (int k = 0; k < ObsPerFrame; k++)
                        wo.Set(e: eo[k], c: new Position { X = f });
                    ...
                    for (int i = 0; i < _ringCount; i++) dacc += _ring[i].x;
                    drainTicks += Stopwatch.GetTimestamp() - t0;
                    drainedTotal += _ringCount;
                }
            }
        );
        double allocKb = (GC.GetAllocatedBytesForCurrentThread() - allocA) / 1024.0;
        _sink += dacc;
        wo.Dispose();

        double drainMs = drainTicks * 1000.0 / Stopwatch.Frequency;
        Report(
            label: $"Set + OnSet observe + enqueue  ({totalSets:n0})",
            ops: totalSets,
            ms: msObs
        );
        double observeOnlyMs = msObs - msBase - drainMs;
        Console.WriteLine(
    ...
    private static (EcsWorld, Entity[]) MakeObsWorld()
    {
        ...
        for (int i = 0; i < ObsEntities; i++)
        {
            ents[i] = w.CreateEntity();
            w.Set(e: ents[i], c: new Position());
        }
    ...
    private static bool GCSettings_IsServer() => GCSettings.IsServerGC;
