...
///     <see cref="Physics" /> / <see cref="Audio" />. A script can spawn/query entities for
///     data-oriented
///     gameplay (10k projectiles in one query instead of 10k Components), read its own node's
///     canonical
///     <c>Transform</c>, or instantiate runtime prefabs.
///     <para>
///         The host assigns <see cref="World" /> / <see cref="Scene" /> / <see cref="Prefabs" /> on
///         play
///         start and clears them on stop; outside play every accessor is null and the helpers are safe
///         no-ops.
...
public static class Ecs
{
    ...
    /// <summary>
    ///     The entity backing an editor node id (<see cref="Entity.Null" /> if unmapped / outside
    ///     play).
    /// </summary>
    public static Entity EntityForNode(int nodeId) => Scene?.EntityOf(nodeId) ?? Entity.Null;

    /// <summary>
    ///     The entity backing the calling component's node (via its <see cref="Component.EntityId" />
    ///     ).
    /// </summary>
    public static Entity EntityFor(Component component) => EntityForNode((int)component.EntityId);

    /// <summary>Spawn a bare entity (<see cref="Entity.Null" /> outside play).</summary>
    public static Entity CreateEntity() => World?.CreateEntity() ?? Entity.Null;

    /// <summary>Instantiate a named runtime prefab (<see cref="Entity.Null" /> if unknown / outside play).</summary>
    public static Entity Instantiate(string prefab) => Prefabs?.Instantiate(prefab) ?? Entity.Null;
}
