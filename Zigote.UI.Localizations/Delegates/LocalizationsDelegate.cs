...
public abstract class LocalizationsDelegate
{
    ...
    public virtual bool ShouldReload(LocalizationsDelegate old) => false;
}
...
public abstract class LocalizationsDelegate<T> : LocalizationsDelegate where T : notnull
{
    ...
    public sealed override object LoadResource(Locale locale) => Load(locale);
}
...
public static class LocalizationsDelegates
{
    ...
    public static LocalizationsDelegate<T> Create<T>(Func<Locale, bool> isSupported,
        Func<Locale, T> load)
        where T : notnull =>
        new FuncDelegate<T>(isSupported: isSupported, load: load);
    ...
    public static LocalizationsDelegate<T> Always<T>(Func<Locale, T> load) where T : notnull =>
        new FuncDelegate<T>(isSupported: static _ => true, load: load);
    ...
    {
        public override bool IsSupported(Locale locale) => isSupported(locale);

        public override T Load(Locale locale) => load(locale);
    }
}
