...
/// </summary>
[Collection("Reactive-serial")] // don't run these in parallel with each other (they saturate the CPU)
public class ReactiveConcurrencyTests
{
    ...
    public void Concurrent_readers_never_see_a_torn_value()
    {
        ...
        var readers = StartReaders(Threads, () =>
        {
            var p = s.Value;
    ...
    public void Batch_transfers_preserve_the_sum_invariant()
    {
        ...
        var stop = false;
        var readers = StartReaders(2, () => Assert.Equal(total, sum.Value), () => Volatile.Read(ref stop));
    ...
    [Fact]
    public void Concurrent_computed_and_effect_lifecycle_during_writes_does_not_crash_or_deadlock()
    {
    ...
    public void A_signal_can_be_set_from_any_thread_while_read_from_another()
    {
        ...
        var readers = StartReaders(Threads / 2, () => seen.Add(s.Value), () => Volatile.Read(ref stop));

        RunThreads(Threads / 2, t =>
        {
    ...
    private static void RunThreads(int n, Action<int> body)
    {
        ...
        Assert.True(Task.WaitAll(tasks, Budget), "threads did not finish within the budget (possible deadlock)");
    }
    ...
    {
        Assert.True(Task.WaitAll(tasks.ToArray(), Budget), "reader tasks did not finish within the budget");
    }
