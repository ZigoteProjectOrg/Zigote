using Xunit;
using Zigote.Core;
using Zigote.Core.Paint;
using Zigote.UI.Widgets;
using Zigote.UI.Widgets.Layout;
using Zigote.UI.Widgets.Navigation;

namespace Zigote.Tests;

/// <summary>
///     Exercises the Navigator stack semantics — push/pop with typed results, named routes,
///     declarative page reconciliation (Navigator 2.0), and replacement — through the public API.
///     Routes use zero-duration transitions so every mutation settles synchronously, keeping the
///     tests deterministic and free of the global <c>Ticker</c>.
/// </summary>
public class NavigatorTests
{
    private static readonly Constraints Tight = Constraints.Tight(width: 200, height: 200);

    private static InstantRoute<T> Page<T>(string? name = null)
    {
        return new InstantRoute<T>(
            builder: _ => new SizedBox(width: 10, height: 10),
            settings: name is null ? null : new RouteSettings(name)
        );
    }

    // Build the navigator into existence: Measure runs the mount lifecycle (OnMount), which
    // creates the initial routes.
    private static Navigator Mount(Navigator nav)
    {
        nav.Measure(Tight);
        return nav;
    }

    [Fact]
    public void Home_BecomesTheInitialRoute()
    {
        var state = Mount(new Navigator { Home = new SizedBox(width: 10, height: 10) });

        Assert.Single(state.History);
        Assert.False(state.CanPop);
    }

    [Fact]
    public void Push_AddsRoute_AndBecomesCurrent()
    {
        var state = Mount(new Navigator { Home = new SizedBox(width: 10, height: 10) });

        var route = Page<object?>();
        state.Push(route);

        Assert.Equal(expected: 2, actual: state.History.Count);
        Assert.True(state.CanPop);
        Assert.Same(expected: route, actual: state.CurrentRoute);
    }

    [Fact]
    public async Task Pop_RemovesRoute_AndCompletesResult()
    {
        var state = Mount(new Navigator { Home = new SizedBox(width: 10, height: 10) });

        var task = state.Push(Page<string>());
        Assert.False(task.IsCompleted);

        state.Pop("done");

        Assert.True(task.IsCompleted);
        Assert.Equal(expected: "done", actual: await task);
        Assert.Single(state.History);
        Assert.False(state.CanPop);
    }

    [Fact]
    public void Pop_OnRootRoute_IsNoOp()
    {
        var state = Mount(new Navigator { Home = new SizedBox(width: 10, height: 10) });

        state.Pop(); // nothing to pop — the base route stays

        Assert.Single(state.History);
    }

    [Fact]
    public async Task Detach_CompletesPendingPushTasks()
    {
        // Regression: tearing the navigator down mid-flow must complete any awaited Push, or the
        // awaiter (and its captured continuation) leaks forever. Unmount previously only DisposeRoute'd.
        var nav = new Navigator { Home = new SizedBox(width: 10, height: 10) };
        var state = Mount(nav);

        var task = state.Push(Page<string>());
        Assert.False(task.IsCompleted);

        state.Detach();

        Assert.True(task.IsCompleted);
        Assert.Null(await task); // no result supplied → completes with default
    }

    [Fact]
    public void PushNamed_ResolvesFromRouteTable()
    {
        var state = Mount(
            new Navigator {
                Home = new SizedBox(width: 10, height: 10),
                Routes = new Dictionary<string, WidgetBuilder> {
                    ["/details"] = _ => new SizedBox(width: 20, height: 20),
                },
            }
        );

        state.PushNamed("/details");

        Assert.Equal(expected: 2, actual: state.History.Count);
        Assert.Equal(expected: "/details", actual: state.CurrentRoute!.Settings.Name);
    }

    [Fact]
    public void InitialRoute_SelectsFromRouteTable()
    {
        var state = Mount(
            new Navigator {
                InitialRoute = "/start",
                Routes = new Dictionary<string, WidgetBuilder> {
                    ["/start"] = _ => new SizedBox(width: 20, height: 20),
                },
            }
        );

        Assert.Single(state.History);
        Assert.Equal(expected: "/start", actual: state.CurrentRoute!.Settings.Name);
    }

    [Fact]
    public void PushReplacement_RemovesReplacedRoute()
    {
        var state = Mount(new Navigator { Home = new SizedBox(width: 10, height: 10) });

        var first = Page<object?>();
        var firstTask = state.Push(first);
        Assert.Equal(expected: 2, actual: state.History.Count);

        state.PushReplacement(Page<object?>());

        // The replaced route is gone (its task completes); the stack height is unchanged.
        Assert.Equal(expected: 2, actual: state.History.Count);
        Assert.True(firstTask.IsCompleted);
        Assert.DoesNotContain(expected: first, collection: state.History);
    }

    [Fact]
    public void PopUntil_UnwindsToNamedRoute()
    {
        var state = Mount(new Navigator { Home = new SizedBox(width: 10, height: 10) });

        state.Push(Page<object?>("a"));
        state.Push(Page<object?>("b"));
        state.Push(Page<object?>("c"));
        Assert.Equal(expected: 4, actual: state.History.Count);

        state.PopUntil(Navigator.WithName("a"));

        Assert.Equal(expected: 2, actual: state.History.Count); // home + "a"
        Assert.Equal(expected: "a", actual: state.CurrentRoute!.Settings.Name);
    }

    [Fact]
    public void Pages_DescribeTheInitialStack()
    {
        var state = Mount(
            new Navigator {
                Pages = [
                    new MaterialPage(new SizedBox(width: 10, height: 10)) {
                        Key = new ValueKey<int>(1),
                    },
                    new MaterialPage(new SizedBox(width: 10, height: 10)) {
                        Key = new ValueKey<int>(2),
                    },
                ],
            }
        );

        Assert.Equal(expected: 2, actual: state.History.Count);
    }

    [Fact]
    public void SetPages_ReconcilesByKey_PreservingRoutes()
    {
        var p1 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(1),
            Animate = false,
        };
        var p2 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(2),
            Animate = false,
        };
        var state = Mount(new Navigator { Pages = [p1, p2] });

        var routeForKey1 = state.History[0];
        var routeForKey2 = state.History[1];

        // Reorder + add a third page. Matched pages must keep their live route instance.
        var p3 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(3),
            Animate = false,
        };
        state.SetPages([p2, p1, p3]);

        Assert.Equal(expected: 3, actual: state.History.Count);
        Assert.Same(expected: routeForKey2, actual: state.History[0]);
        Assert.Same(expected: routeForKey1, actual: state.History[1]);
        Assert.NotSame(expected: routeForKey1, actual: state.History[2]);
        Assert.NotSame(expected: routeForKey2, actual: state.History[2]);
    }

    [Fact]
    public void SetPages_RemovingPage_DropsItsRoute()
    {
        var p1 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(1),
            Animate = false,
        };
        var p2 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(2),
            Animate = false,
        };
        var state = Mount(new Navigator { Pages = [p1, p2] });
        var dropped = state.History[1];

        state.SetPages([p1]);

        Assert.Single(state.History);
        Assert.DoesNotContain(expected: dropped, collection: state.History);
    }

    [Fact]
    public void RenderPath_RunsMeasureLayoutPaint_AcrossTheStack()
    {
        var nav = new Navigator { Home = new SizedBox(width: 10, height: 10) };
        var state = Mount(nav);
        state.Push(Page<object?>());

        // A full Measure → Layout → Paint cycle must run the route stack (build + attach + lay out new
        // content) without error, and the navigator fills the window.
        var size = nav.Measure(Tight);
        nav.Layout(Offset.Zero);
        nav.Paint(new PaintList());

        Assert.Equal(expected: 200f, actual: size.Width, precision: 2);
        Assert.Equal(expected: 200f, actual: size.Height, precision: 2);
        Assert.Equal(expected: 2, actual: state.History.Count);
    }

    [Fact]
    public void OnPopPage_CanVetoAPagePop()
    {
        var p1 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(1),
            Animate = false,
        };
        var p2 = new MaterialPage(new SizedBox(width: 10, height: 10)) {
            Key = new ValueKey<int>(2),
            Animate = false,
        };

        bool allow = false;
        var state = Mount(
            new Navigator {
                Pages = [p1, p2],
                OnPopPage = (_, _) => allow,
            }
        );

        state.Pop(); // vetoed
        Assert.Equal(expected: 2, actual: state.History.Count);

        allow = true;
        state.Pop(); // permitted
        Assert.Single(state.History);
    }

    // A page route with no transition — push/pop/replace complete in the same call.
    private sealed class InstantRoute<T>(WidgetBuilder builder, RouteSettings? settings = null)
        : MaterialPageRoute<T>(builder: builder, settings: settings)
    {
        public override float TransitionDuration => 0f;
    }
}
