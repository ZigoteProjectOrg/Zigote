...
internal sealed class VideoPage : ComposedWidget
{
                ...
                new Column(
                    ...
                    [
                        ...
                        new Wrap(
                            ...
                            children:
                            [
                                new OutlinedButton(new Text("Open a file…"), OpenFile),
                                new OutlinedButton(new Text("Local sample"), LoadSample),
                                new OutlinedButton(
                                    new Text("Stream over HTTPS"),
                                    () => Load(HttpsSample)
                                ),
                                new OutlinedButton(new Text("Stream HLS"), () => Load(HlsSample)),
                            ]
                        ),
                    ]
                )
            ...
            Section(
                ...
                new Column(
                    ...
                    [
                        ...
                        new SizedBox(height: 12),
                        LabeledRow(
                            new Zigote.UI.Material.Switch(
                                _player.Loop.Value,
                                v => _player.Loop.Value = v
                            ),
                            "Loop"
                        ),
                    ]
    ...
    {
        _sourceLabel.Text = _status ?? _player.Error.Value ?? Describe();
    }
    ...
    private string Describe()
    {
        ...
        var video = media.Video is { } v
            ...
            : "no video";
        var audio = media.Audio is { } a
            ? $"{a.Codec} {a.SampleRate} Hz ×{a.Channels}"
            : "no audio";

        var name = media.IsNetwork ? media.Source : Path.GetFileName(media.Source);
        var length = media.IsLive ? "live" : $"{media.Duration:mm\\:ss}";
        var transport = media.IsNetwork ? " · streaming" : "";
    ...
    private void OpenFile()
    {
        ...
        async void Run()
        {
            try
            {
                var picked = await FileBrowserDialog.ShowAsync(
                    app,
                    new FileBrowserOptions {
                        Kind = FileDialogKind.OpenFile,
                        Title = "Open a video",
                        Filters = [new FileDialogFilter("Media", VideoExtensions)],
                    }
                );
    ...
    private async Task Open(string source, TimeSpan resumeAt = default)
    {
        Report(null);
        _source = source;
        await _player.OpenAsync(source, Qualities[_quality].MaxHeight);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "zigote-gallery-sample.mp4");
        ...
        foreach (var a in new[]
                 {
                     ...
                     "-f", "lavfi", "-i", "testsrc2=size=960x540:rate=30:duration=15",
