...
public sealed class GraphDomainRegistry
{
    ...
    {
        if (!_domains.TryGetValue(domainId, out var d))
    ...
    {
        return _nodes.TryGetValue(definitionId, out var n) ? n : null;
    }

    public IEnumerable<NodeDefinition> NodeDefinitionsForDomain(string domainId)
    {
        return _nodes.Values.Where(n => n.DomainId == domainId);
    }
    ...
    public IEnumerable<NodeDefinition> Search(string query, string? domainId = null)
    {
        ...
        return source.Where(n =>
            n.DisplayName.Contains(lower, StringComparison.OrdinalIgnoreCase) ||
            n.Category.Contains(lower, StringComparison.OrdinalIgnoreCase) ||
            n.Tags.Any(t => t.Contains(lower, StringComparison.OrdinalIgnoreCase))
        );
    }
}