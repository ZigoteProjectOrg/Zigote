...
/// <summary>
///     A server-authoritative replicated object. Subclass it, declare <see cref="NetVar{T}" /> fields
///     and
///     register them in the constructor with <see cref="Register{TVar}" />; the server streams their
///     changes
///     to clients, which instantiate a matching subclass (by <see cref="PrefabId" />) and apply the
///     state.
///     Override the lifecycle hooks for spawn/despawn glue and <see cref="Interpolate" /> for
///     smoothing.
/// </summary>
public abstract class NetObject
{
    ...
    /// <summary>
    ///     True on the client instance that owns this object (its local player drives it via
    ///     prediction).
    /// </summary>
    public bool IsOwner { get; internal set; }

    /// <summary>
    ///     The server holds authority; the owning client may still predict locally (see the
    ///     prediction helpers).
    /// </summary>
    public bool HasAuthority => IsServer;

    /// <summary>
    ///     The registered replicated fields, in declaration order (the order both ends must agree
    ///     on).
    /// </summary>
    ...
    {
        for (int i = 0; i < _vars.Count; i++) _vars[i].Serialize(writer);
    }

    internal void ApplyFull(NetReader reader)
    {
        for (int i = 0; i < _vars.Count; i++) _vars[i].Deserialize(reader);
    }

    internal void SerializeDelta(NetWriter writer)
    {
        for (int i = 0; i < _vars.Count; i++) writer.WriteBool(_vars[i].IsDirty);
        for (int i = 0; i < _vars.Count; i++)
        {
            if (_vars[i].IsDirty)
    ...
    internal void ApplyDelta(NetReader reader)
    {
        ...
        for (int i = 0; i < _vars.Count; i++) dirty[i] = reader.ReadBool();
        for (int i = 0; i < _vars.Count; i++)
        {
            if (dirty[i])
    ...
    {
        for (int i = 0; i < _vars.Count; i++) _vars[i].Tick();
    }

    /// <summary>
    ///     Server: called once on the frame the object is spawned. Client: called after instantiation
    ///     + first full state.
    /// </summary>
    ...
    /// <summary>
    ///     Client hook to advance any interpolation toward <paramref name="renderTime" /> (server
    ///     render timeline).
    /// </summary>
