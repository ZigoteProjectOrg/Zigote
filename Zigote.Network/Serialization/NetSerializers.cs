...
public static class NetSerializers
{
    ...
    public static Vec2 ReadVec2(this NetReader r) => new(x: r.ReadSingle(), y: r.ReadSingle());
    ...
    public static Vec2 ReadRangedVec2(this NetReader r, float min, float max, int bits) => new(
        x: r.ReadRangedSingle(min: min, max: max, bits: bits),
        y: r.ReadRangedSingle(min: min, max: max, bits: bits)
    );
    ...
    public static Vec3 ReadVec3(this NetReader r) => new(
        x: r.ReadSingle(),
        y: r.ReadSingle(),
        z: r.ReadSingle()
    );
    ...
    public static Vec3 ReadRangedVec3(this NetReader r, float min, float max, int bits)
    {
        return new Vec3(
            x: r.ReadRangedSingle(min: min, max: max, bits: bits),
            y: r.ReadRangedSingle(min: min, max: max, bits: bits),
            z: r.ReadRangedSingle(min: min, max: max, bits: bits)
        );
    }
    ...
    public static void WriteQuaternion(this NetWriter w, Quat q, int bitsPerComponent = 10)
    {
        ...
        w.WriteBits(value: (uint)largest, bits: 2);
        if (largest != 0)
        {
            w.WriteRangedSingle(
                value: x * sign,
                min: -QuatComponentRange,
                max: QuatComponentRange,
    ...
    public static Quat ReadQuaternion(this NetReader r, int bitsPerComponent = 10)
    {
        ...
        float sumSq = (a * a) + (b * b) + (c * c) + (d * d);
        float dropped = MathF.Sqrt(MathF.Max(x: 0f, y: 1f - sumSq));
    ...
    {
        w.WriteByte((byte)Math.Clamp(value: (int)MathF.Round(c.R * 255f), min: 0, max: 255));
        w.WriteByte((byte)Math.Clamp(value: (int)MathF.Round(c.G * 255f), min: 0, max: 255));
        w.WriteByte((byte)Math.Clamp(value: (int)MathF.Round(c.B * 255f), min: 0, max: 255));
        w.WriteByte((byte)Math.Clamp(value: (int)MathF.Round(c.A * 255f), min: 0, max: 255));
    }
    ...
    public static Transform3D ReadTransform(this NetReader r) => new(
        position: r.ReadVec3(),
    ...
    // ── NetId ────────────────────────────────────────────────────────────────
    public static void WriteNetId(this NetWriter w, NetId id) => w.WriteVarUInt(id.Value);

    public static NetId ReadNetId(this NetReader r) => new(r.ReadVarUInt());
}
