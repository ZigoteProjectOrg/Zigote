...
/// <summary>
///     Ordered ring of un-acknowledged client inputs. The client keeps inputs here after sending them
///     so it
///     can replay the still-unacked ones when the server's authoritative state arrives. Acking by
///     sequence
...
public sealed class InputBuffer<TInput> where TInput : IInputCommand
{
    ...
    public void Add(TInput input) => _inputs.Add(input);
    ...
    public void AckThrough(uint sequence)
    {
        ...
        if (keep > 0) _inputs.RemoveRange(index: 0, count: keep);
    }
