...
/// <summary>
///     Client-side prediction with server reconciliation. The game supplies a deterministic step
///     function
///     <c>(state, input, dt) → state</c>. Each tick, <see cref="Predict" /> stamps the input, advances
///     the
///     local state immediately (so control feels instant) and buffers the input. When the server's
///     authoritative state for a processed input sequence arrives, <see cref="Reconcile" /> snaps to
///     it and
///     replays the inputs the server hasn't acknowledged yet — correcting prediction error without
///     losing
///     responsiveness. The step function MUST match the server's (same math, same dt) or prediction
///     will
...
public sealed class ClientPrediction<TInput, TState> where TInput : IInputCommand
{
    ...
    /// <summary>
    ///     Stamp the input, predict locally, buffer it, and return the stamped command to send to the
    ///     server.
    /// </summary>
    public TInput Predict(TInput input, float dt)
    {
        input.Sequence = ++LastInputSequence;
        State = _step(arg1: State, arg2: input, arg3: dt);
        _buffer.Add(input);
        return input;
    }

    /// <summary>
    ///     Apply the server's authoritative state (which reflects inputs up to
    ///     <paramref name="lastProcessedSequence" />)
    ///     and replay the rest.
    /// </summary>
    public void Reconcile(TState authoritative, uint lastProcessedSequence, float dt)
    {
        ...
        for (int i = 0; i < pending.Count; i++)
            State = _step(arg1: State, arg2: pending[i], arg3: dt);
    }
