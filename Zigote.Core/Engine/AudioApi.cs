...
///     have to stub them.
///     <para>
///         It is an interface for one reason worth the indirection: <b>the device is the one part of
///         a player that cannot exist in CI</b>. A queue, a transport and an equalizer are pure state
...
public interface IAudioApi
{
    ...
    ///     the rate actually achieved (0 = failure, sound now disabled). <b>Every sound, bus and
    ///     equalizer id becomes invalid.</b> The rate is fixed at device creation, so playing a
...
internal sealed class EngineAudioApi(ZigoteEngine engine) : IAudioApi
{
    ...
    {
        return engine.AudioReopen(sampleRateHz);
    }

    public uint CreateFile(string path, bool streaming)
    {
        return engine.AudioSoundCreateFile(path, streaming);
    }

    public uint CreateStream()
    {
        return engine.AudioStreamCreate();
    }

    public int StreamPush(uint id, ReadOnlySpan<byte> bytes)
    {
        return engine.AudioStreamPush(id, bytes);
    }

    public void StreamFinish(uint id)
    {
        engine.AudioStreamFinish(id);
    }

    public AudioStreamState StreamStatus(uint id)
    {
        return engine.AudioStreamStatus(id);
    }

    public float StreamBuffered(uint id)
    {
        return engine.AudioStreamBuffered(id);
    }

    public void Destroy(uint id)
    {
        engine.AudioSoundDestroy(id);
    }
    ...
    {
        engine.AudioSoundStop(id);
    }

    public void Seek(uint id, float seconds)
    {
        engine.AudioSoundSeek(id, seconds);
    }

    public void ScheduleStart(uint id, float secondsFromNow)
    {
        engine.AudioSoundScheduleStart(id, secondsFromNow);
    }

    public float Cursor(uint id)
    {
        return engine.AudioSoundCursor(id);
    }

    public float Duration(uint id)
    {
        return engine.AudioSoundDuration(id);
    }

    public bool IsPlaying(uint id)
    {
        return engine.AudioSoundIsPlaying(id);
    }

    public bool AtEnd(uint id)
    {
        return engine.AudioSoundAtEnd(id);
    }
    ...
    {
        engine.AudioSoundSetSpatial(id, enabled);
    }

    public void SetEq(uint id, uint eqId)
    {
        engine.AudioSoundSetEq(id, eqId);
    }

    public uint EqCreate(int bandCount)
    {
        return engine.AudioEqCreate(bandCount);
    }
    ...
    {
        engine.AudioEqSetEnabled(eqId, enabled);
    }

    public void EqDestroy(uint eqId)
    {
        engine.AudioEqDestroy(eqId);
    }

    public float[] DecodeFile(string path, out int channels, out int sampleRate)
    {
        return engine.AudioDecodeFile(path, out channels, out sampleRate);
    }
}