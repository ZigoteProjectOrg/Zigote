...
///         share them), and <c>OperationsPerInvoke</c> is that same constant — so the Mean column
///         reads
///         directly as <em>nanoseconds per operation</em> and is comparable straight down a
///         benchmark's
...
///         sharing at all — anything above flat here is pure gate cost), one shared written signal
///         (the
...
///     <para>
///         Run:
///         <c>dotnet run -c Release --project Zigote.Reactive.Benchmark -- --filter *Contention*</c>
///     </para>
/// </summary>
[MemoryDiagnoser]
...
public class ContentionBenchmarks
{
    ...
    public void Setup()
    {
        ...
        _sharedLive = _sharedDerived.Observe(() => { });
    }
    ...
    public void DisjointGraphWrites()
    {
        ...
        Parallel.For(
            ...
            {
                var s = _disjoint[t];
                for (int i = 0; i < perThread; i++) s.Value = i;
            }
        );
    }
    ...
    public void SharedSignalWrites()
    {
        ...
        Parallel.For(
            ...
            {
                int offset = t << 20;
                for (int i = 0; i < perThread; i++) _sharedWrite.Value = offset | i;
            }
        );
    }
    ...
    public int SharedSignalReads()
    {
        ...
        Parallel.For(
            ...
            {
                int local = 0;
                for (int i = 0; i < perThread; i++) local += _sharedRead.Value;
...
{
    public ContentionConfig() => AddJob(Job.ShortRun);
}
